...
def math_compleks_namber(data:str):
    ...
    def math_operations(list_data:list):
        ...
        while True:
            try:
                float(list_data[0][0])
                float(list_data[0][1])
                if len (list_data)>1 and list_data.count('*')==0 and list_data.count('/')==0 and list_data.count('+')==0 and list_data.count('-')==0:
                    err='no signs'
                    return ch.alarm_cneck(str(err))
                
                if list_data.count('*')>0 or list_data.count('/')>0:
                     if list_data.count('*')>0 and list_data.count('/')>0 and (list_data.index('*')<list_data.index('/')):
                        attr='*'
                     elif list_data.count('*')>0 and list_data.count('/')>0 and (list_data.index('/')<list_data.index('+')):
                        attr='/'
